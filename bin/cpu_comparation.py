...
results_gpu = convolution.sync_convolve(positions=pos)
results_cpu = convolution.cpu_convolve(psf=psf,
                                       positions=pos,
                                       image_size=image_size,
                                       image_pixel_size=image_pixel_size,
                                       psf_pixel_size=psf_pixel_size)
...
for i in range(n):
    ...
    ax3.imshow(np.abs(results_gpu[i] - results_cpu[i]) / np.max(np.abs(results_gpu[i])))
